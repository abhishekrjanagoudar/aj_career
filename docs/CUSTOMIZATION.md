...
## CV Template (LaTeX: AJ CV/resume.tex)

You can use your LaTeX template as the primary PDF engine:
...
```bash
node generate-pdf-latex.mjs "AJ CV/resume.tex" output/cv-candidate-example-latex.pdf --format=a4
```

Add role-specific ATS keywords without changing your core experience:

```bash
node generate-pdf-latex.mjs "AJ CV/resume.tex" output/cv-candidate-example-latex.pdf \
  --format=letter \
  --keywords="ROS 2,SLAM,LiDAR,VDA 5050,MQTT,AAS" \
  --job-title="Working Student Robotics" \
