...
## Motor de template (HTML / LaTeX / BOTH)

Soporta 3 motores:

- `latex` (default): usa plantilla del usuario en `AJ CV/resume.tex` + `AJ CV/resume.cls`
- `html`: usa `templates/cv-template.html`
- `both`: genera ambos PDFs en paralelo (HTML + LaTeX)

Si el usuario no especifica motor, usar `latex` por defecto.
...
13. Escribe assets temporales:
   - LaTeX: `/tmp/cv-candidate-{company}.tex`
   - HTML: `/tmp/cv-candidate-{company}.html`
14. Ejecuta según motor:
   - `latex`:
     `node generate-pdf-latex.mjs "AJ CV/resume.tex" output/cv-candidate-{company}-{YYYY-MM-DD}-latex.pdf --format={letter|a4} --keywords="kw1,kw2,kw3" --job-title="{role}" --company="{company}"`
   - `html`:
     `node generate-pdf.mjs /tmp/cv-candidate-{company}.html output/cv-candidate-{company}-{YYYY-MM-DD}-html.pdf --format={letter|a4}`
   - `both`: ejecutar ambos comandos
15. Reporta: rutas de PDF(s), motor usado, % cobertura de keywords
...
## Template LaTeX (AJ CV)

Usar plantilla del usuario:
...
- Comando: `node generate-pdf-latex.mjs`
